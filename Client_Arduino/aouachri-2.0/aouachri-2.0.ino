/*
 * ============================================
 * ESP8266 OTA Client - Mise à jour Over-The-Air (V2.0)
 * PROJET: AOUACHRI - VERSION AVEC SOLDE 50%
 * ============================================
 */
...
// ============================================
const char* defaultVersion = "aouachri-2.0"; // ✅ Version 2 par défaut
...
// ============================================
#define LCD_I2C_ADDRESS 0x27  // ✅ Correction : 0x27 (conforme à votre projet antérieur)
...
void updateLCDDisplay(const String& version) {
    ...
    // ✅ AFFICHAGE SPÉCIFIQUE POUR LA VERSION 2.0 : SOLDE 50%
    if (versionNumber.startsWith("2.") || versionNumber == "2.0") {
        lcd.setCursor(0, 0);
        lcd.print("SOLDE 50% !!!");
        lcd.setCursor(1, 1);
        lcd.print("PRIX 50 DT");
    }
    // Optionnel : garder V1 pour compatibilité descendante
    ...
    // Autres versions (ex: 3.0)
    else if (versionNumber == "3.0") {
        lcd.setCursor(0, 0);
        lcd.print("V3 EN ATTENTE");
        lcd.setCursor(0, 1);
...
bool performOTAUpdate(const char* firmwareURL, const String& newVersion, const String& newHash) {
    ...
    ESPhttpUpdate.onProgress([](int current, int total) {
        ...
        if (percent != lastPercent && (percent % 10 == 0 || percent == 100)) {
            ...
            yield(); // ✅ Évite les watchdog resets
        }
    });
